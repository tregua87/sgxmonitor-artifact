...
typedef struct ms_my_exception_t {
	int ms_retval;
	int ms_i;
} ms_my_exception_t;
...
static sgx_status_t SGX_CDECL sgx_my_exception(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_my_exception_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_my_exception_t* ms = SGX_CAST(ms_my_exception_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = my_exception(ms->ms_i);
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_my_exception, 0},
