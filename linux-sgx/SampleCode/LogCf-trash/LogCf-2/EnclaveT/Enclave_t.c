...
{
	CHECK_REF_POINTER(pms, sizeof(ms_makeEndMsg_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_makeEndMsg_t* ms = SGX_CAST(ms_makeEndMsg_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	unsigned char* _tmp_strO = ms->ms_strO;
	size_t _tmp_lenO = ms->ms_lenO;
	size_t _len_strO = _tmp_lenO * sizeof(unsigned char);
	unsigned char* _in_strO = NULL;

	if (sizeof(*_tmp_strO) != 0 &&
		(size_t)_tmp_lenO > (SIZE_MAX / sizeof(*_tmp_strO))) {
		return SGX_ERROR_INVALID_PARAMETER;
	}

	CHECK_UNIQUE_POINTER(_tmp_strO, _len_strO);
	...
	if (_tmp_strO != NULL && _len_strO != 0) {
		if ( _len_strO % sizeof(*_tmp_strO) != 0)
		{
			status = SGX_ERROR_INVALID_PARAMETER;
			goto err;
		}
		if ((_in_strO = (unsigned char*)malloc(_len_strO)) == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		memset((void*)_in_strO, 0, _len_strO);
	}

	makeEndMsg(_in_strO, _tmp_lenO);
	if (_in_strO) {
		if (memcpy_s(_tmp_strO, _len_strO, _in_strO, _len_strO)) {
