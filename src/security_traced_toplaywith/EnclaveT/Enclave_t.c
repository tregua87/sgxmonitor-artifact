...
static sgx_status_t SGX_CDECL sgx_topwn(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_topwn_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_topwn_t* ms = SGX_CAST(ms_topwn_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	char* _tmp_buff = ms->ms_buff;



	topwn(_tmp_buff, ms->ms_buff_len);
