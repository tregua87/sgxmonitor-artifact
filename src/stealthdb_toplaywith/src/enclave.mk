...
XCLANG=-Xclang -load -Xclang $(LLVM_PATH)/build/lib/LLVMTracerPass.so
...
FLAGS:= -m64 -O0 -g -fvisibility=hidden -fpie -fno-stack-protector -fno-builtin-printf -Wall -Wextra -Wpedantic $(XCLANG)
CFLAGS := $(FLAGS) $(CPPFLAGS) -nostdinc -std=c11
CXXFLAGS :=  $(FLAGS) $(CPPFLAGS) -nostdinc++ -std=c++11

LDFLAGS := \
	-Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(SDKI_INSTALL_PATH)/lib64\
...
$(ENCLAVE_DIR)/enclave_t.c: $(SGX_EDGER8R) $(ENCLAVE_DIR)/enclave.edl
	@cd $(ENCLAVE_DIR) && $(SGX_EDGER8R) --trusted enclave.edl --search-path $(SGX_INCLUDE_PATH)
	@mv $(ENCLAVE_DIR)/enclave_t.h include/enclave
	@echo "GEN  =>  $@"

$(ENCLAVE_DIR)/enclave_t.o: $(ENCLAVE_DIR)/enclave_t.c
	@$(CC) -Iinclude/enclave $(CFLAGS) -c $< -o $@ -I../../../inc
	@echo "CC  <=  $<"
...
$(TARGET): $(ENCLAVE_DIR)/enclave_t.o $(CXX_OBJS) $(ASM_OBJS) $(C_OBJS)  ../../tracer/T-Tracer.o
	@$(CXX) $^ -o $@ $(LDFLAGS)
	@echo "LINK =>  $@"
