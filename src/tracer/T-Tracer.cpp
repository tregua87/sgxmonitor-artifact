...
// XXX: the worst way to instrument a module ever! :) open to suggestion...
void* __NOT_INSTRUMENT_THIS__ = 0;

// I suppose these functions are implemented
// in the enclave that will include the Tracer
extern "C" sgx_status_t SGX_CDECL ocall_print_string(const char* str);
extern "C" sgx_status_t SGX_CDECL ocall_monitorgatewayu(const char* strI, size_t lenI, char* strO, size_t lenO);

unsigned short TRACER_ON = 0; // 0 => false
                              // 1 => true

#define EDGE_SIZE 17

extern uint8_t __ImageBase;
void* ba = (void*)&__ImageBase;
...
static bucket_t* pub_bucket = NULL;
static int* actionCounter = NULL;

void writeToBuffer(unsigned char*, size_t);
void writeToBuffer2(unsigned char*, size_t, entry_status_t);
void nextNonce(void);
extern void elaborate(char,void*,void*);
...
extern "C" void makeEndMsg() {
  ...
  writeToBuffer2(ct, OPT_KEY_SIZE_BYTE, RED);

}

extern "C" void setBucket(bucket_t *b) {
  pub_bucket = b;
}

extern "C" void setActionCounter(int *ac) {
  actionCounter = ac;
}

extern "C" void bootSecureCommunication(void) {
  if (TRACER_ON)
    return;
  char ret[BuffSize] = { 0 };
...
// void traceassigmentf(void* ptr, void* val) {
void traceassigmentf(void* val) {
  void* from =  __builtin_return_address(0);
  elaborate('A', from, val);
}

void traceframe(void* frm) {
  elaborate('F', frm, 0);
}

void traceptr(void* obj, void* vtbl) {
  elaborate('V', obj, vtbl);
}
...
extern void elaborate(char type, void* pOp1, void* pOp2) {
  if (!TRACER_ON)
    return;
  ...
  void* op1 = (void*)((unsigned long)pOp1 - (unsigned long)ba);
  void* op2 = (void*)((unsigned long) pOp2 - (unsigned long)ba);
  ...
  buf_pt[0] = type;
  memcpy(buf_pt+1, &op1, sizeof(void*));
  memcpy(buf_pt+1+sizeof(void*), &op2, sizeof(void*));
...
static char buf_pt[OPT_KEY_SIZE_BYTE] = { 0 };
static unsigned int i = 0;
static unsigned char tmpp[EDGE_SIZE + OPT_KEY_SIZE_BYTE] = { 0 };
static unsigned char tmpc[OPT_KEY_MAC_SIZE_BYTE] = { 0 };

extern void elaborateDumb(char type, void* pOp1, void* pOp2) {
  if (!TRACER_ON)
    return;
  ...
  // unsigned char ct[OPT_KEY_SIZE_BYTE] = { 0 }; // this seems sminkiare the canary dio cane why?!?!?
  // unsigned char ct[10] = { 0 };
  // unsigned char ct = 'A';
...
extern sgx_thread_mutex_t global_mutex = SGX_THREAD_MUTEX_INITIALIZER;

void writeToBuffer(unsigned char *str, size_t str_len) {
  writeToBuffer2(str, str_len, BLACK);
}

void writeToBuffer2(unsigned char *str, size_t str_len, entry_status_t A_COLOR) {
