...
static sgx_status_t SGX_CDECL sgx_setBucketM(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_setBucketM_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_setBucketM_t* ms = SGX_CAST(ms_setBucketM_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	setBucketM(_tmp_b, _tmp_exit_loop);
...
SGX_EXTERNC const struct {
	...
	5,
	{
		...
		{(void*)(uintptr_t)sgx_setBucketM, 0},
	}
};
