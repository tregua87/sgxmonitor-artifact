...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < MAX_TEST; i++) {
		RUN_AND_DUMP(MODE, "init_store", init_store(eid))
		RUN_AND_DUMP(MODE, "add_to_store", add_to_store(eid, bytes_in, MAX_BUF_LEN))
		RUN_AND_DUMP(MODE, "get_from_store", get_from_store(eid, bytes_out, MAX_BUF_LEN, 0))
		RUN_AND_DUMP(MODE, "encrypt_store", encrypt_store(eid, "afile.bin"))
		RUN_AND_DUMP(MODE, "decrypt_store", decrypt_store(eid, afile_bin, afile_bin_len))
		RUN_AND_DUMP(MODE, "free_store", free_store(eid))
		RUN_AND_DUMP(MODE, "store_to_bytes", store_to_bytes(eid))
    }
