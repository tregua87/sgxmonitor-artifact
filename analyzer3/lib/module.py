...
def passf_random(state):
    state.regs.flags = state.solver.Unconstrained("random", state.arch.bits)
...
def skip_call(state):
    print(" THIS IS A CALL, SKIP IT")
    iv = state.block().vex
    state.regs.rip = state.regs.rip + iv.size

# just exit
class MyCloseExploration(SimProcedure):
    def __init__(self, isTraced = False):
        super(MyCloseExploration, self).__init__()
        self.isTraced = isTraced
        
    def run(self, *args, **kwargs):
        
        if self.isTraced:
            self.state.my_tracer.add("E", "0x0", "0x0")
...
def loop_handling(state):
    ...
    ss_ctx = 0 # hashlib.sha256(str(ss).encode()).hexdigest()
...
class MyStubFunc(SimProcedure):
    def __init__(self, isTraced):
        super(MyStubFunc, self).__init__()
        self.isTraced = isTraced
    ...
    def run(self, *args, **kwargs):
        ...
        if self.isTraced:
            ...
            # if self.state.addr == 0x1220:
            #     from IPython import embed; embed()
            # #     dst = hex(self.state.callstack[1].current_return_target)

            self.state.my_tracer.add("E", "0x0", src)
...
class MyTracerHook(SimProcedure):
    def __init__(self, actionType):
        super(MyTracerHook, self).__init__()
    ...
    def run(self, *args, **kwargs):
        ...
        if dst != '0x0' and dst != '0x1':
            sym = self.project.loader.find_symbol(int(dst, 16))
            if sym is not None and sym.is_function:
                if sym.name in self.state.globals["traced_functions"]:
                    tag = sym.name
                else:
                    tag = "SKIP"
        ...
        self.state.my_tracer.add(self.actionType, src, dst, tag)
