...
from angr.knowledge_plugins.cfg import CFGNode, MemoryDataSort, MemoryData
from angr.knowledge_plugins.xrefs import XRef, XRefType
from angr.misc.ux import deprecated
from angr import sim_options as o
from angr.errors import (AngrCFGError, AngrSkipJobNotice, AngrUnsupportedSyscallError, SimEngineError, SimMemoryError,
...
from angr.analyses.cfg.cfg_arch_options import CFGArchOptions
        ...
        try:
            ...
            if nodecode or irsb.size == 0 or irsb.jumpkind == 'Ijk_NoDecode':
                ...
                if is_x86_x64_arch \
                    ...
                    nodecode_size = 2
                #
                # elif is_x86_x64_arch \
                #         and len(irsb_string) >= 3 \
                #         and irsb_string[-3:] in {
                #         	  # 0f  01  d7
                #             b'\x0f\x01\xd7',  # enclu
                #         }:
                #     l.error("enclue gotcha!")
                #     # enclu is actually a valid instruction.
                #     valid_ins = True
                #     nodecode_size = 3
                else:
                    valid_ins = False
                ...
                # if addr + irsb_size in (0x23941, 0x23a6e):
                #     l.error("MY ADDR [{:x}]".format(addr + irsb_size))
                #     l.error("is_x86_x64_arch {}".format(is_x86_x64_arch))
                #     l.error(irsb_string.hex())
                #     l.error("len: {}".format(len(irsb_string)))
                #     l.error("irsb_string[-3:] {}".format(irsb_string[-3:].hex()))
                #     l.error("lifted_block.bytes {}".format(lifted_block.bytes.hex()))
